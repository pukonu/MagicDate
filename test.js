...
test('the first day of week of a given week from a date object', () => {
    ...
    expect(MagicDate.makeDate(CASE1).getWeekFirstDate().toString()).toMatch('2018-03-18')
    expect(MagicDate.makeDate(CASE2).getWeekFirstDate().toString()).toMatch('2018-06-17')
})

test('the last day of week of a given week from a date object', () => {
    // case1 should assert to Saturday - March 24, 2018
    // case2 should assert to Sunday - June 23, 2018
    expect(MagicDate.makeDate(CASE1).getWeekLastDate().toString()).toMatch('2018-03-24')
    expect(MagicDate.makeDate(CASE2).getWeekLastDate().toString()).toMatch('2018-06-23')
})

test('get 3 days ahead from a reference date', () => {
    // case1 should assert to Saturday - March 21, 2018
    // case2 should assert to Sunday - June 23, 2018
    expect(MagicDate.makeDate(CASE1).next("3 days").toString()).toMatch('2018-03-21')
    expect(MagicDate.makeDate(CASE2).next("3 days").toString()).toMatch('2018-06-23')
})

test('get 3 prior from a reference date', () => {
    // case1 should assert to Saturday - March 15, 2018
    // case2 should assert to Sunday - June 17, 2018
    expect(MagicDate.makeDate(CASE1).next("-3 days").toString()).toMatch('2018-03-15')
    expect(MagicDate.makeDate(CASE2).next("-3 days").toString()).toMatch('2018-06-17')
})
...
test('get 3 weeks ahead from a reference date', () => {
    ...
    expect(MagicDate.makeDate(CASE1).next("3 weeks").toString()).toMatch('2018-04-08')
    expect(MagicDate.makeDate(CASE2).next("3 weeks").toString()).toMatch('2018-07-11')
})
...
test('get 1 month prior from a reference date', () => {
    ...
    expect(MagicDate.makeDate({
        year: 2018,
        month: 3,
        day: 29
    }).next("-1 month").toDateString()).toMatch('2018-02-28')
})
...
test('get 2 years and a month prior from a reference date', () => {
    ...
    expect(MagicDate.makeDate({
        ...
        month: 3,
        day: 31
    }).next("-2 years").next("-1 month").toDateString()).toMatch('2016-02-29')
})

test('get the week number based on a given date', () => {
    // console.log(DateUtil.makeDate().getWeekNum());
    expect(MagicDate
        .makeDate(CASE1) // week starts on Sunday by default
        .getWeekNum()
    ).toEqual(11);
...
test('get dates from a week number', () => {
    ...
    const weekArr = MagicDate
        .getDatesFromWeekNum(1, 2018)
        .map(v => v.setCast('%Y-%m-%d').toString())
